...
function Call() {
    ...
    if (URLParams) {
        let remoteId = URLParams.get("pid");
        let uid = URLParams.get("uid");
        if (remoteId && peerId != remoteId && remotePeerIdValue.length == 0) {
    ...
    useEffect(() => {
        if (remotePeerIdValue.length > 0 && peerInstance) {
            navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then((mediaStream) => {
                currentUserVideoRef.current!.srcObject = mediaStream;
                currentUserVideoRef.current!.play();
    ...
    }, [remotePeerIdValue, peerInstance])
